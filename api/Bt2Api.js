...
function parseRecvData(value) {
  ...
  if (endIndex == -1 || btCommandIndex == -1) {
    return
  }
  if (value.indexOf('OK') == 0)
    _data.recvState = 'OK'
  else if (value.indexOf('NG') == 0)
    _data.recvState = 'NG'
  else {
    _data.recvCommand = value.substr(0, btCommandIndex - 1)
  }
...
class Bt2Api {
  ...
  static login() {
    return sendReq(_cmd.login)
  }

  static getControlSerial(serial) {
    return sendReq(_cmd.getSerial, serial)
  }
  static setControlSerial(serial) {
    return sendReq(_cmd.setSerial, serial)
  }
  static delControlSerial(serial) {
    return sendReq(_cmd.delSerial, serial)
  }
  static setEstateNum(num) {
    return sendReq(_cmd.setEstateNum, num)
  }

  static getEstateNum() {
    return sendReq(_cmd.getEstateNum)
  }
  static setWifissid(wifissid) {
    return sendReq(_cmd.setWifissid, wifissid)
  }

  static getWifissid() {
    return sendReq(_cmd.getWifissid)
  }
  static setWifipsw(wifipsw) {
    return sendReq(_cmd.setWifipsw, wifipsw)
  }

  static getWifipsw() {
    return sendReq(_cmd.getWifipsw)
  }

  static setVol(vol) {
    return sendReq(_cmd.setVol, vol)
  }

  static getVol() {
    return sendReq(_cmd.getVol)
  }

  static setRit(rit) {
    return sendReq(_cmd.setRit, rit)
  }

  static getRit() {
    return sendReq(_cmd.getRit)
  }

  static setCtc(rit) {
    return sendReq(_cmd.setCtc, rit)
  }

  static getCtc() {
    return sendReq(_cmd.getCtc)
  }
